...
def preprocess_dataframe(df):
    ...
    Returns:
        pd.DataFrame: The preprocessed DataFrame with features ready for model training.
    """
    
    expected_columns = ['FLAG_OWN_CAR', 'WEEKDAY_APPR_PROCESS_START', 'FLAG_OWN_REALTY', 
                        'OCCUPATION_TYPE', 'NAME_TYPE_SUITE', 'EMERGENCYSTATE_MODE', 
                        'NAME_CONTRACT_TYPE', 'WALLSMATERIAL_MODE', 'ORGANIZATION_TYPE', 
                        'NAME_INCOME_TYPE', 'NAME_FAMILY_STATUS', 'CODE_GENDER', 
                        'NAME_EDUCATION_TYPE', 'NAME_HOUSING_TYPE', 'HOUSETYPE_MODE', 
                        'FONDKAPREMONT_MODE']
    
    # Vérifier si des colonnes attendues manquent dans le DataFrame
    missing_columns = [col for col in expected_columns if col not in df.columns]
    if missing_columns:
        print("Attention: Colonnes manquantes dans le DataFrame d'entrée:", missing_columns)
    ...
    # Get the new feature names after transformation (handling one-hot encoded columns)
    new_feature_names = []
    ...
    # Add back the 'SK_ID_CURR' column without preprocessing (since it's an identifier)
    if 'SK_ID_CURR' in df.columns:
        df_transformed['SK_ID_CURR'] = df['SK_ID_CURR']
...
def load_model():
    ...
    mlflow.set_tracking_uri("https://dagshub.com/davisouattara/Project_7_Models.mflow.mlflow/")
    # Define the model URI from the provided information
    #model_uri = 'runs:/71c3bbf459bb4be6a639f6abfb6f8c74/model'
    model_uri = 'runs:/7d0fc938e1cd42f2bc4926c51ed87600/model'
...
def get_final_estimator(pipeline):
    ...
    """
    if hasattr(pipeline, "steps"):  # Vérifie si c'est un pipeline
        final_estimator = pipeline.steps[-1][1]  # Récupère le dernier modèle
    else:
        final_estimator = pipeline  # C'est déjà un modèle

    # Vérifie si c'est un modèle XGBoost
    if hasattr(final_estimator, "get_booster"):  
        print("✅ Modèle XGBoost détecté !")
    else:
        print("⚠️ Ce n'est pas un modèle XGBoost. Vérifie la compatibilité avec SHAP.")
...
def main():
    ...
    # with col2:
    #st.image("P7_image.PNG", width=700)  # Adjust width as desired
    ...
    if sk_id_curr in customer_data_preprocessed['SK_ID_CURR'].values:
        ...
        **{gender_pronoun.capitalize()}** is a **{age_years}** years old **{input_data_copy['NAME_EDUCATION_TYPE']}** who works in the **{input_data_copy['NAME_INCOME_TYPE']}** sector. **{gender_pronoun.capitalize()}** lives in a **{housing_type}** and is currently **{employment_duration_years}** years into employment. **{gender_pronoun.capitalize()}** is **{family_status}** and has applied for a **{input_data_copy['NAME_CONTRACT_TYPE'].lower()}** loan. **{gender2_pronoun.capitalize()}** income is **{input_data_copy['AMT_INCOME_TOTAL']}** €. 
        ...
        input_data1 = customer_data_copy[customer_data_copy['SK_ID_CURR'] == sk_id_curr].iloc[0].to_dict()
        input_df0 = pd.DataFrame([input_data])
        ...
        if prediction_label == "Refused":  
            ...
            model_columns = model.feature_names_in_  # Si tu utilises un modèle scikit-learn
